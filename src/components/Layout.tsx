...
const SearchBox = memo(function SearchBox({
  ...
  return (
    <form onSubmit={submit} className="relative">
      ...
      {results.length > 0 && (
        <div className="absolute left-0 right-0 top-full z-50 mt-2 max-h-96 overflow-auto rounded-2xl border border-border bg-popover p-2 shadow-luxury">
          {results.map((r) => (
            <Link
              key={r.id}
              to="/product/$id"
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 md:px-8">
        <Link to="/" className="flex items-center gap-2">
          ...
          <div className="leading-tight">
            <div className="font-serif text-xl font-semibold tracking-wider text-primary">
              PANCHHI
            </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="relative">
            ...
            {acctOpen && (
              <>
                ...
                <div className="absolute right-0 top-full z-50 mt-2 w-56 rounded-2xl border border-border bg-popover p-2 shadow-luxury">
                  {user ? (
                    <>
          ...
          <Link
            to="/book"
            className="hidden rounded-full bg-primary px-5 py-2 text-sm font-medium text-primary-foreground shadow-soft transition-transform hover:scale-105 lg:inline-flex"
          >
            Book
          </Link>
  ...
  return (
    <footer className="mt-24 bg-[var(--maroon-deep)] text-primary-foreground">
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:grid-cols-4 md:px-8">
        <div>
          ...
          <p className="mt-4 text-sm opacity-75">
            A premium digital showroom — buy, rent and book your
            perfect outfit for every occasion.
          </p>
        </div>

        <div>
          <h4 className="font-serif text-lg text-[var(--gold)]">
            Shop
          </h4>
          ...
          <ul className="mt-3 space-y-2 text-sm opacity-80">
            ...
            <li>
              <Link
                to="/collection"
                search={{ category: "Party Wear" } as never}
              >
                Party Wear
              </Link>
            </li>
        ...
        <div>
          ...
          <ul className="mt-3 space-y-2 text-sm opacity-80">
            ...
            <li className="flex gap-2">
              ...
              <span>hello@panchhi.in</span>
            </li>
          </ul>

          <div className="mt-4 flex gap-3">
            <a className="rounded-full border border-white/20 p-2 hover:bg-white/10">
      ...
      <div className="border-t border-white/10 py-5 text-center text-xs opacity-60">
        © Panchhi By Abha Collection · Since 2006
      </div>
    </footer>
  );
});
...
export const WhatsAppFab = memo(function WhatsAppFab() {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="fixed bottom-5 right-5 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] shadow-luxury"
    >
      <svg viewBox="0 0 32 32" className="h-7 w-7 fill-white">
        <path d="M19.11 17.21c-.27-.13-1.6-.79-1.85-.88-.25-.09-.43-.13-.61.13-.18.27-.7.88-.86 1.06-.16.18-.32.2-.59.07-.27-.13-1.14-.42-2.17-1.34-.8-.71-1.34-1.6-1.5-1.87-.16-.27-.02-.41.12-.54.12-.12.27-.32.4-.48.13-.16.18-.27.27-.45.09-.18.04-.34-.02-.47-.07-.13-.61-1.47-.84-2.01-.22-.53-.45-.46-.61-.47h-.52c-.18 0-.47.07-.72.34-.25.27-.95.93-.95 2.27 0 1.34.97 2.63 1.11 2.81.13.18 1.92 2.93 4.66 4.11.65.28 1.16.45 1.55.58.65.21 1.24.18 1.71.11.52-.08 1.6-.65 1.83-1.28.23-.63.23-1.17.16-1.28-.07-.11-.25-.18-.52-.31zM16 3a13 13 0 0 0-11.04 19.86L3 29l6.36-1.95A13 13 0 1 0 16 3z" />
      </svg>
    </a>
  );
});

export const StickyMobileBar = memo(function StickyMobileBar() {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-30 grid grid-cols-2 gap-2 border-t border-border bg-background/95 p-3 md:hidden">
      <Link
        to="/cart"
        className="rounded-full border border-primary py-2.5 text-center text-sm font-medium text-primary"
      >
        View Cart
      </Link>
      ...
      <Link
        to="/book"
        className="rounded-full bg-primary py-2.5 text-center text-sm font-medium text-primary-foreground"
      >
        Book Trial
      </Link>
    </div>
